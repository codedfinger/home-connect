...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (profile.role !== "admin") {
      setLocation(
        profile.role === "tenant" ? "/dashboard/tenant" : "/"
      );
    }
    ...
    <div className="min-h-screen flex flex-col bg-muted/10">
      ...
      <div className="bg-primary text-primary-foreground py-12 px-4 md:px-6">
        <div className="container mx-auto">
          <h1 className="text-3xl font-display font-bold">Admin — Listings</h1>
          ...
          <Tabs defaultValue="properties" className="w-full">
            ...
            <TabsContent value="properties" className="p-6 md:p-8 m-0">
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-xl font-bold font-display">Manage properties</h2>
              ...
              {isLoadingProps ? (
                ...
                <div className="grid gap-6">
                  {properties.map((prop) => (
                    <div
                      key={prop.id}
                      className="flex flex-col md:flex-row gap-6 p-4 rounded-2xl border border-border hover:shadow-md transition-shadow bg-white"
                    >
                      ...
                      <div className="flex-grow flex flex-col justify-between">
                        <div>
                          <div className="flex items-start justify-between">
                            ...
                            <span
                              className={`px-2.5 py-1 text-xs font-bold rounded-md uppercase tracking-wide ${prop.status === "available" ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}
                            >
                              {prop.status}
                            </span>
                          </div>
                          <p className="text-muted-foreground text-sm flex items-center mt-1">
                            <MapPin className="w-4 h-4 mr-1" /> {prop.city} •{" "}
                            {formatNaira(prop.price, prop.type, "mo")}
                          </p>
                        </div>
                        <div className="flex items-center gap-3 mt-4 md:mt-0">
                          <Link href={`/dashboard/admin/properties/edit/${prop.id}`}>
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (confirm("Delete this listing?")) deleteProp({ id: prop.id });
                            }}
                          >
                    ...
                    <div key={enq.id} className="bg-white p-6 rounded-2xl border border-border shadow-sm">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <Link
                            href={`/listings/${enq.propertyId}`}
                            className="text-primary text-sm hover:underline font-medium mt-1 inline-block"
                          >
