...
async function buildEnquiry(e, includeProperty, includeTenant) {
  ...
  return {
    ...
    property,
    tenant,
    createdAt: e.createdAt?.toISOString() ?? (/* @__PURE__ */ new Date()).toISOString()
  };
}
...
router.post("/enquiries", async (req, res) => {
  ...
  const [created] = await db.insert(enquiriesTable).values({ ...parsed.data, tenantId: req.user.id, phone: parsed.data.phone ?? null }).returning();
...
router.get("/enquiries/received", async (req, res) => {
  ...
  const myPropertyIds = await db.select({ id: propertiesTable.id }).from(propertiesTable).where(eq(propertiesTable.landlordId, req.user.id));
  const ids = myPropertyIds.map((p) => p.id);
  if (!ids.length) {
    res.json({ enquiries: [], total: 0 });
    return;
  }
  const enqs = await db.select().from(enquiriesTable).where(sql`${enquiriesTable.propertyId} = ANY(ARRAY[${sql.join(ids.map((id) => sql`${id}`), sql`, `)}]::int[])`);
