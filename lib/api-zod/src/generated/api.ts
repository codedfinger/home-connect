/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * HomeConnect Real Estate API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from "zod";

/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  status: zod.string(),
});

/**
 * @summary Get current authenticated user
 */
export const GetCurrentAuthUserResponse = zod.object({
  user: zod
    .object({
      id: zod.string(),
      username: zod.string(),
      firstName: zod.string().nullish(),
      lastName: zod.string().nullish(),
      profileImage: zod.string().nullish(),
    })
    .nullable(),
});

/**
 * @summary Redirect to login
 */
export const LoginQueryParams = zod.object({
  returnTo: zod.coerce.string().optional(),
});

/**
 * @summary Get current user profile with role
 */
export const GetUserProfileResponse = zod.object({
  id: zod.string(),
  username: zod.string(),
  firstName: zod.string().nullish(),
  lastName: zod.string().nullish(),
  profileImage: zod.string().nullish(),
  role: zod.enum(["landlord", "tenant", "admin"]).nullish(),
  phone: zod.string().nullish(),
  isVerified: zod.boolean(),
  bio: zod.string().nullish(),
});

/**
 * @summary Update user profile (set role, phone, etc.)
 */
export const UpdateUserProfileBody = zod.object({
  phone: zod.string().nullish(),
  bio: zod.string().nullish(),
});

export const UpdateUserProfileResponse = zod.object({
  id: zod.string(),
  username: zod.string(),
  firstName: zod.string().nullish(),
  lastName: zod.string().nullish(),
  profileImage: zod.string().nullish(),
  role: zod.enum(["landlord", "tenant", "admin"]).nullish(),
  phone: zod.string().nullish(),
  isVerified: zod.boolean(),
  bio: zod.string().nullish(),
});

/**
 * @summary List all available properties with optional filters
 */
export const ListPropertiesQueryParams = zod.object({
  type: zod.enum(["rent", "sale"]).optional(),
  city: zod.coerce.string().optional(),
  minPrice: zod.coerce.number().optional(),
  maxPrice: zod.coerce.number().optional(),
  bedrooms: zod.coerce.number().optional(),
  verified: zod.coerce.boolean().optional(),
});

export const ListPropertiesResponse = zod.object({
  properties: zod.array(
    zod.object({
      id: zod.number(),
      landlordId: zod.string().nullish(),
      listingContactName: zod.string().nullish(),
      listingContactPhone: zod.string().nullish(),
      title: zod.string(),
      description: zod.string(),
      type: zod.enum(["rent", "sale"]),
      status: zod.enum(["available", "rented", "sold"]),
      price: zod.number(),
      city: zod.string(),
      address: zod.string(),
      bedrooms: zod.number(),
      bathrooms: zod.number(),
      area: zod.number().nullish(),
      images: zod.array(zod.string()),
      isVerified: zod.boolean(),
      hasLandDocuments: zod.boolean(),
      landlord: zod
        .object({
          id: zod.string(),
          username: zod.string(),
          firstName: zod.string().nullish(),
          lastName: zod.string().nullish(),
          profileImage: zod.string().nullish(),
          phone: zod.string().nullish(),
          isVerified: zod.boolean(),
        })
        .nullish(),
      isBookmarked: zod.boolean(),
      createdAt: zod.coerce.date(),
    }),
  ),
  total: zod.number(),
});

/**
 * @summary Create a new property listing (landlord only)
 */
export const CreatePropertyBody = zod.object({
  title: zod.string(),
  description: zod.string(),
  type: zod.enum(["rent", "sale"]),
  price: zod.number(),
  city: zod.string(),
  address: zod.string(),
  bedrooms: zod.number(),
  bathrooms: zod.number(),
  area: zod.number().nullish(),
  images: zod.array(zod.string()),
  hasLandDocuments: zod.boolean(),
  listingContactName: zod.string(),
  listingContactPhone: zod.string(),
});

/**
 * @summary Get a single property by ID
 */
export const GetPropertyParams = zod.object({
  id: zod.coerce.number(),
});

export const GetPropertyResponse = zod.object({
  id: zod.number(),
  landlordId: zod.string().nullish(),
  listingContactName: zod.string().nullish(),
  listingContactPhone: zod.string().nullish(),
  title: zod.string(),
  description: zod.string(),
  type: zod.enum(["rent", "sale"]),
  status: zod.enum(["available", "rented", "sold"]),
  price: zod.number(),
  city: zod.string(),
  address: zod.string(),
  bedrooms: zod.number(),
  bathrooms: zod.number(),
  area: zod.number().nullish(),
  images: zod.array(zod.string()),
  isVerified: zod.boolean(),
  hasLandDocuments: zod.boolean(),
  landlord: zod
    .object({
      id: zod.string(),
      username: zod.string(),
      firstName: zod.string().nullish(),
      lastName: zod.string().nullish(),
      profileImage: zod.string().nullish(),
      phone: zod.string().nullish(),
      isVerified: zod.boolean(),
    })
    .nullish(),
  isBookmarked: zod.boolean(),
  createdAt: zod.coerce.date(),
});

/**
 * @summary Update a property listing (owner only)
 */
export const UpdatePropertyParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdatePropertyBody = zod.object({
  title: zod.string().optional(),
  description: zod.string().optional(),
  type: zod.enum(["rent", "sale"]).optional(),
  status: zod.enum(["available", "rented", "sold"]).optional(),
  price: zod.number().optional(),
  city: zod.string().optional(),
  address: zod.string().optional(),
  bedrooms: zod.number().optional(),
  bathrooms: zod.number().optional(),
  area: zod.number().nullish(),
  images: zod.array(zod.string()).optional(),
  hasLandDocuments: zod.boolean().optional(),
  listingContactName: zod.string().optional(),
  listingContactPhone: zod.string().optional(),
});

export const UpdatePropertyResponse = zod.object({
  id: zod.number(),
  landlordId: zod.string().nullish(),
  listingContactName: zod.string().nullish(),
  listingContactPhone: zod.string().nullish(),
  title: zod.string(),
  description: zod.string(),
  type: zod.enum(["rent", "sale"]),
  status: zod.enum(["available", "rented", "sold"]),
  price: zod.number(),
  city: zod.string(),
  address: zod.string(),
  bedrooms: zod.number(),
  bathrooms: zod.number(),
  area: zod.number().nullish(),
  images: zod.array(zod.string()),
  isVerified: zod.boolean(),
  hasLandDocuments: zod.boolean(),
  landlord: zod
    .object({
      id: zod.string(),
      username: zod.string(),
      firstName: zod.string().nullish(),
      lastName: zod.string().nullish(),
      profileImage: zod.string().nullish(),
      phone: zod.string().nullish(),
      isVerified: zod.boolean(),
    })
    .nullish(),
  isBookmarked: zod.boolean(),
  createdAt: zod.coerce.date(),
});

/**
 * @summary Delete a property listing (owner only)
 */
export const DeletePropertyParams = zod.object({
  id: zod.coerce.number(),
});

export const DeletePropertyResponse = zod.object({
  success: zod.boolean(),
  message: zod.string().nullish(),
});

/**
 * @summary Admin — all listings; landlord — own listings only
 */
export const GetMyPropertiesResponse = zod.object({
  properties: zod.array(
    zod.object({
      id: zod.number(),
      landlordId: zod.string().nullish(),
      listingContactName: zod.string().nullish(),
      listingContactPhone: zod.string().nullish(),
      title: zod.string(),
      description: zod.string(),
      type: zod.enum(["rent", "sale"]),
      status: zod.enum(["available", "rented", "sold"]),
      price: zod.number(),
      city: zod.string(),
      address: zod.string(),
      bedrooms: zod.number(),
      bathrooms: zod.number(),
      area: zod.number().nullish(),
      images: zod.array(zod.string()),
      isVerified: zod.boolean(),
      hasLandDocuments: zod.boolean(),
      landlord: zod
        .object({
          id: zod.string(),
          username: zod.string(),
          firstName: zod.string().nullish(),
          lastName: zod.string().nullish(),
          profileImage: zod.string().nullish(),
          phone: zod.string().nullish(),
          isVerified: zod.boolean(),
        })
        .nullish(),
      isBookmarked: zod.boolean(),
      createdAt: zod.coerce.date(),
    }),
  ),
  total: zod.number(),
});

/**
 * @summary Send an enquiry about a property (tenant)
 */
export const CreateEnquiryBody = zod.object({
  propertyId: zod.number(),
  message: zod.string(),
  phone: zod.string().nullish(),
});

/**
 * @summary Get all enquiries received by the landlord
 */
export const GetReceivedEnquiriesResponse = zod.object({
  enquiries: zod.array(
    zod.object({
      id: zod.number(),
      propertyId: zod.number(),
      tenantId: zod.string(),
      message: zod.string(),
      phone: zod.string().nullish(),
      status: zod.enum(["pending", "read"]),
      property: zod
        .object({
          id: zod.number(),
          landlordId: zod.string().nullish(),
          listingContactName: zod.string().nullish(),
          listingContactPhone: zod.string().nullish(),
          title: zod.string(),
          description: zod.string(),
          type: zod.enum(["rent", "sale"]),
          status: zod.enum(["available", "rented", "sold"]),
          price: zod.number(),
          city: zod.string(),
          address: zod.string(),
          bedrooms: zod.number(),
          bathrooms: zod.number(),
          area: zod.number().nullish(),
          images: zod.array(zod.string()),
          isVerified: zod.boolean(),
          hasLandDocuments: zod.boolean(),
          landlord: zod
            .object({
              id: zod.string(),
              username: zod.string(),
              firstName: zod.string().nullish(),
              lastName: zod.string().nullish(),
              profileImage: zod.string().nullish(),
              phone: zod.string().nullish(),
              isVerified: zod.boolean(),
            })
            .nullish(),
          isBookmarked: zod.boolean(),
          createdAt: zod.coerce.date(),
        })
        .nullish(),
      tenant: zod
        .object({
          id: zod.string(),
          username: zod.string(),
          firstName: zod.string().nullish(),
          lastName: zod.string().nullish(),
          profileImage: zod.string().nullish(),
          phone: zod.string().nullish(),
          isVerified: zod.boolean(),
        })
        .nullish(),
      createdAt: zod.coerce.date(),
    }),
  ),
  total: zod.number(),
});

/**
 * @summary Get all enquiries sent by the current tenant
 */
export const GetSentEnquiriesResponse = zod.object({
  enquiries: zod.array(
    zod.object({
      id: zod.number(),
      propertyId: zod.number(),
      tenantId: zod.string(),
      message: zod.string(),
      phone: zod.string().nullish(),
      status: zod.enum(["pending", "read"]),
      property: zod
        .object({
          id: zod.number(),
          landlordId: zod.string().nullish(),
          listingContactName: zod.string().nullish(),
          listingContactPhone: zod.string().nullish(),
          title: zod.string(),
          description: zod.string(),
          type: zod.enum(["rent", "sale"]),
          status: zod.enum(["available", "rented", "sold"]),
          price: zod.number(),
          city: zod.string(),
          address: zod.string(),
          bedrooms: zod.number(),
          bathrooms: zod.number(),
          area: zod.number().nullish(),
          images: zod.array(zod.string()),
          isVerified: zod.boolean(),
          hasLandDocuments: zod.boolean(),
          landlord: zod
            .object({
              id: zod.string(),
              username: zod.string(),
              firstName: zod.string().nullish(),
              lastName: zod.string().nullish(),
              profileImage: zod.string().nullish(),
              phone: zod.string().nullish(),
              isVerified: zod.boolean(),
            })
            .nullish(),
          isBookmarked: zod.boolean(),
          createdAt: zod.coerce.date(),
        })
        .nullish(),
      tenant: zod
        .object({
          id: zod.string(),
          username: zod.string(),
          firstName: zod.string().nullish(),
          lastName: zod.string().nullish(),
          profileImage: zod.string().nullish(),
          phone: zod.string().nullish(),
          isVerified: zod.boolean(),
        })
        .nullish(),
      createdAt: zod.coerce.date(),
    }),
  ),
  total: zod.number(),
});

/**
 * @summary Get all bookmarked properties for the current user
 */
export const GetBookmarksResponse = zod.object({
  properties: zod.array(
    zod.object({
      id: zod.number(),
      landlordId: zod.string().nullish(),
      listingContactName: zod.string().nullish(),
      listingContactPhone: zod.string().nullish(),
      title: zod.string(),
      description: zod.string(),
      type: zod.enum(["rent", "sale"]),
      status: zod.enum(["available", "rented", "sold"]),
      price: zod.number(),
      city: zod.string(),
      address: zod.string(),
      bedrooms: zod.number(),
      bathrooms: zod.number(),
      area: zod.number().nullish(),
      images: zod.array(zod.string()),
      isVerified: zod.boolean(),
      hasLandDocuments: zod.boolean(),
      landlord: zod
        .object({
          id: zod.string(),
          username: zod.string(),
          firstName: zod.string().nullish(),
          lastName: zod.string().nullish(),
          profileImage: zod.string().nullish(),
          phone: zod.string().nullish(),
          isVerified: zod.boolean(),
        })
        .nullish(),
      isBookmarked: zod.boolean(),
      createdAt: zod.coerce.date(),
    }),
  ),
  total: zod.number(),
});

/**
 * @summary Bookmark a property
 */
export const AddBookmarkBody = zod.object({
  propertyId: zod.number(),
});

/**
 * @summary Remove a bookmark
 */
export const RemoveBookmarkParams = zod.object({
  propertyId: zod.coerce.number(),
});

export const RemoveBookmarkResponse = zod.object({
  success: zod.boolean(),
  message: zod.string().nullish(),
});
