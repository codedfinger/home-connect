            ...
            <TabsContent value="enquiries" className="p-6 md:p-8 m-0">
              {isLoadingEnq ? (
                ...
                <div className="grid gap-4 max-w-4xl">
                  {enquiries.map(enq => (
                    <div key={enq.id} className="flex flex-col sm:flex-row gap-4 p-5 rounded-2xl border border-border hover:shadow-md transition-shadow bg-white items-start">
                      <div className="w-full sm:w-24 h-24 rounded-xl overflow-hidden bg-muted flex-shrink-0">
                        {enq.property?.images?.[0] ? (
                          <img src={enq.property.images[0]} alt="" className="w-full h-full object-cover" />
                        ) : (
