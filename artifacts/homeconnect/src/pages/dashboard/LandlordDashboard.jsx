...
function LandlordDashboard() {
  ...
  >
            ...
            <TabsContent value="properties" className="p-6 md:p-8 m-0">
              ...
              {isLoadingProps ? <div className="space-y-4">
                  {[1, 2].map((i) => <div key={i} className="h-24 bg-muted animate-pulse rounded-2xl" />)}
                </div> : properties.length === 0 ? <div className="text-center py-16 bg-muted/30 rounded-2xl border border-dashed border-border">
                  <p className="text-muted-foreground mb-4">You haven't listed any properties yet.</p>
                  <Link href="/dashboard/landlord/properties/new">
                    <Button variant="outline" className="rounded-xl">Create your first listing</Button>
                  </Link>
                </div> : <div className="grid gap-6">
                  {properties.map((prop) => <div key={prop.id} className="flex flex-col md:flex-row gap-6 p-4 rounded-2xl border border-border hover:shadow-md transition-shadow bg-white">
                      ...
                      <div className="flex-grow flex flex-col justify-between">
                        <div>
                          <div className="flex items-start justify-between">
                            ...
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-md uppercase tracking-wide ${prop.status === "available" ? "bg-emerald-100 text-emerald-800" : "bg-muted text-muted-foreground"}`}>
                              {prop.status}
                            </span>
                          </div>
  ...
  >
            ...
            <TabsContent value="enquiries" className="p-6 md:p-8 m-0 bg-muted/5">
              ...
              {isLoadingEnq ? <div className="space-y-4">
                ...
                </div> : enquiries.length === 0 ? <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-border">
                  <MessageSquare className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-muted-foreground">No enquiries received yet.</p>
                </div> : <div className="grid gap-4">
                  {enquiries.map((enq) => <div key={enq.id} className="bg-white p-6 rounded-2xl border border-border shadow-sm">
                      ...
                      {enq.phone && <div className="inline-flex items-center text-sm font-medium px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg">
