...
export default function PropertyFormPage() {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      bathrooms: 1,
      area: null,
      images: [] as any, // handled by transform
  ...
  useEffect(() => {
    if (property && isEditing) {
      form.reset({
        ...property,
        images: property.images?.join(', ') as any, // string for text input
      });
    }
  ...
  const onSubmit = (values: FormValues) => {
    if (isEditing) {
      updateProp({ id: propertyId, data: values as any });
    } else {
      createProp({ data: values as any });
    }
  };
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2 bg-emerald-50 border border-emerald-100 p-6 rounded-2xl">
                  <FormField control={form.control} name="hasLandDocuments" render={({ field }) => (
