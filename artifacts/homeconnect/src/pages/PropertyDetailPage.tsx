    ...
    <div className="min-h-screen bg-muted/10">
      ...
      <div className="container mx-auto px-4 py-8 animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 h-[500px] bg-muted rounded-3xl"></div>
          ...
          <div>
            <div className="flex flex-wrap gap-2 mb-3">
              ...
              {property.status !== 'available' && (
                <span className="px-3 py-1 bg-destructive text-destructive-foreground text-xs font-bold rounded-full uppercase tracking-wider">
                  {property.status}
                </span>
              )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-12 h-[40vh] md:h-[60vh]">
          ...
          <div className="hidden md:flex flex-col gap-4 h-full">
            {images.slice(1, 3).map((img, i) => (
              <div key={i} className="flex-1 rounded-3xl overflow-hidden relative">
                <img src={img} alt="" className="w-full h-full object-cover" />
              </div>
            ))}
            {images.length <= 1 && (
              <div className="flex-1 rounded-3xl overflow-hidden bg-muted flex items-center justify-center border border-border">
              ...
              <div className="flex items-center gap-4 mb-6 p-4 rounded-2xl bg-muted/30">
                <div className="h-16 w-16 rounded-full bg-primary/10 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm">
                  {property.landlord?.profileImage ? (
                    <img src={property.landlord.profileImage} alt="Owner" className="w-full h-full object-cover" />
                  ) : (
