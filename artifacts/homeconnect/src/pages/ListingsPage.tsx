...
export default function ListingsPage() {
  ...
  const [type, setType] = useState<"rent" | "sale" | "any">("any");
  const [verifiedOnly, setVerifiedOnly] = useState(false);

  // Debounce could be added here for a real app, keeping it simple for MVP
  const { data, isLoading } = useListProperties({
    ...
    query: {
      ...
      queryKey: ["properties", city, type, verifiedOnly],
      queryFn: () => {
        // Just using the hook's query setup pattern from the generic hook
        // We will pass the params to the actual hook call below
        return fetch('/api').then(r=>r.json()); // Dummy, won't be used since we use the hook properly below
      }
    }
  });

  // Re-fetch with exact params
    ...
    <div className="flex flex-col min-h-screen bg-muted/10">
      ...
      <main className="flex-grow container mx-auto px-4 md:px-6 py-8">
        ...
        {realIsLoading ? (
          ...
          <div className="text-center py-32 bg-white rounded-3xl border border-border/50 shadow-sm max-w-2xl mx-auto">
            ...
            <Button variant="outline" onClick={() => { setCity(""); setType("any"); setVerifiedOnly(false); }} className="rounded-xl">
              Clear all filters
            </Button>
          </div>
        )}
