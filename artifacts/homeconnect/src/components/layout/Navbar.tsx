      ...
      <div className="container mx-auto px-4 md:px-6 flex h-16 items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {!isFullyLoaded ? (
              ...
              <DropdownMenuContent className="w-56" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    ...
                    {profile?.role && (
                      <span className="inline-flex items-center rounded-full bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary mt-2 w-max capitalize">
                        {profile.role}
                      </span>
                    )}
                  </div>
            ...
            <Button onClick={() => login()} className="rounded-full shadow-md hover:shadow-lg transition-all font-semibold px-6">
              Sign In
            </Button>
          )}
